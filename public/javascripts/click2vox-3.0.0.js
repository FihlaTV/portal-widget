...
(function(click2vox) {
  ...
  requirejs.config({
    paths: {
      ...
      voxbone: "//cdn.voxbone.com/voxbone/voxbone-3.0.min"
    }
  });
  ...
  if (!voxButtonElements) {
    requirejs(['voxbone'],
      function() {
        voxbone = new Voxbone({postLogs: true});
        loadAssets();
      }
    );
  }
  ...
  function makeCall() {
    if (isInCall() || failedAuthenticating()) return;
    ...
    if (isWebRTCSupported() && !isChromeOnHttp()) {
      ...
      voxbone.WebRTC.configuration.uri = 'sip:' + (caller_id).replace(/[^a-zA-Z0-9-_]/g, '').toString() + '@voxbone.com';
  ...
  function isInCall() {
    return voxbone.WebRTC.rtcSession.isEstablished && !voxbone.WebRTC.rtcSession.isEnded;
  }
  ...
  function callAction(message){
    if (!voxbone.WebRTC.rtcSession.isEstablished || voxbone.WebRTC.rtcSession.isEnded)
      return;
  ...
  function failedAuthenticating() {
    return infoVoxbone.auth_failure === 'true';
  }
